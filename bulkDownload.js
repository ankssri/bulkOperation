...
// Step 1: Execute code to get URLs of Files (Settings --> Files)
async function getFilesBulkOperation() {
    var body = { // Form Bulk Mutation query
        query: `mutation {
        bulkOperationRunQuery(
        query: """
        {
           files (first : ${maxBatchLimit}) { 
              edges {
                node {
                  ... on MediaImage {
                    image {
...
//Step 4 convert .jsonl file to json format
// Refer to filejsonlTojson.js , generated json file name : <storename>_bulkOperation.json
